...
Vortext stores provider API keys in the macOS Keychain with device-only, unlocked-only accessibility. Keychain protects credentials at rest on an uncompromised Mac; Vortext does not claim protection against administrator-level malware, keyloggers, provider compromise, or live process-memory inspection. Reports about prompt injection through clipboard, app, or screen context are in scope: Vortext treats that content as untrusted data but does not represent its mitigations as a complete guarantee.
