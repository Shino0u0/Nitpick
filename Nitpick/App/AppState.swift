...
final class AppState {
    ...
    var activeAction: NitpickAction = .dictate
    ...
    func startAction(_ action: NitpickAction) {
    ...
    private func persist(
        outcome: PipelineOutcome, action: NitpickAction, mode: ModeInput
    ) {
    ...
    private func toggleFromShortcut(_ action: NitpickAction) {
        if recorderState == .recording {
            stopAndProcess()
        } else {
            startAction(action)
